...
/**
 *createkey function should take in the team information record pointer
 *and should return the array of keys for better access (pointers)
 *
 *algorithm below
 *
 *ptr createKey(tir, pointer to keys array){
    create a char pointer to keys
    allocate space for the hash ptr
    check if space allocated correctly for hash ptr
    assign the key in the function to the key in hash table
    conduct the operations using strcat and strcpy using a for loop
    place those values into the pointer to keys array
    return the pointer to the keys in the hash table 
    }
*/
...
HashPtr createKey(TeamInfoPtr_t tir, HashPtr keys[]){
    ...
    ptrHash = (HashPtr)malloc(sizeof(ht_item));

    if (!ptrHash){
        printf("Space for the Hash Pointer not allocatd correctly\n");
        exit(1);
    }
    ...
    char *ptrKeys = malloc(10);
    ptrHash->key = ptrKeys;

    ptrKeys = strcpy(ptrKeys, get_league_str(tir->league));
    ptrKeys = strcat(ptrKeys, tir->nickname); 

    for(index = 0; keys[index] != NULL; index++);
        keys[index] = malloc(sizeof(HashPtr));
        keys[index]->key = ptrHash->key;
        keys[index]->value = tir;
    return ptrHash;
 ...
 *
 */
void ht_insert(ht_hash_table* ht, const char* key,  TeamInfoPtr_t value) {
